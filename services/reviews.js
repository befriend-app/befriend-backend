...
function setActivityReview(activityData = {}, personFromData = {}, personToData = {}, no_show, review, on_save_from_network = false) {
    return new Promise(async (resolve, reject) => {
        try {
            let personActivity, activity_review_id;
            ...
            if (typeof no_show === 'boolean') {
                if (organizedExisting.no_show && organizedExisting.no_show.value !== null) {
                    activity_review_id = organizedExisting.no_show.id;
                    ...
                    //create new record
                    [activity_review_id] = await conn('activities_persons_reviews').insert({
                ...
                if (existingReview) {
                    activity_review_id = existingReview.id;
                    ...
                    [activity_review_id] = await conn('activities_persons_reviews')
                        .insert({
                            person_from_id: personFrom.id,
                            person_to_id: personTo.id,
                            activity_id: activity.activity_id,
            ...
            if (networkSelf.is_befriend) { //update rating score if Befriend domain
                if (typeof no_show === 'boolean') {
                    returnData = await updatePersonNoShow(personTo);
                } else {
                    returnData = await updatePersonRatings(personTo);
                }
            } else if(networkSelf.id === activity.network_id && !on_save_from_network) { //send rating to Befriend
