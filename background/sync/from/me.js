                ...
                for (let section in person.me) {
                    ...
                    for (let token in items) {
                        ...
                        if (section_table.cache_key) {
                            ...
                            lookupPipelines[section].hGet(
                                section_table.cache_key_hash(item.hash_token),
                                item.token,
                            );
                        }
                    }
                ...
                function organizeItemsCache(table, items) {
                    ...
                    for (let item of items) {
                        ...
                        let cache_item = (person_cache[tableInfo.section_key][db_item.token] = {
                            ...
                            ...(tableInfo.col_secondary && {
                                secondary: item[tableInfo.col_secondary] || null,
                            }),
                            updated: item.updated,
                        });
                ...
                for (let table_name in existingData) {
                    ...
                    if (missingIds.size > 0) {
                        if (tableInfo.source_table) {
                            try {
                                let options = await conn(tableInfo.source_table).whereIn(
                                    'id',
                ...
                for (let person of batch) {
                    ...
                    if (person.sections) {
                        for (const [token, section] of Object.entries(person.sections)) {
                            ...
                            let existingSection =
                                existingDataLookup.persons_sections[person_token]?.[token];
                            ...
                            if (existingSection) {
                                if (
                                    section.updated > existingSection.updated ||
                                    debug_sync_enabled
                                ) {
                    ...
                    if (person.me) {
                        for (let [table, items] of Object.entries(person.me)) {
                            ...
                            for (let [token, item] of Object.entries(items)) {
                                ...
                                if (tableInfo.col_secondary) {
                                    itemData[tableInfo.col_secondary] =
                                        item[tableInfo.col_secondary];
                                }
